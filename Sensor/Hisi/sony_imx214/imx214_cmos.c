...
#if !defined(__IMX214_CMOS_H_)
#define __IMX214_CMOS_H_
...
ISP_SNS_STATE_S *g_pastImx214[ISP_MAX_PIPE_NUM] = {HI_NULL};

#define IMX214_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastImx214[dev])
#define IMX214_SENSOR_SET_CTX(dev, pstCtx)   (g_pastImx214[dev] = pstCtx)
#define IMX214_SENSOR_RESET_CTX(dev)         (g_pastImx214[dev] = HI_NULL)

ISP_SNS_COMMBUS_U g_aunImx214BusInfo[ISP_MAX_PIPE_NUM] = {
    [0] = { .s8I2cDev = 0},
...
typedef struct hiIMX214_STATE_S {
    HI_U8       u8Hcg;
    HI_U32      u32BRL;
    HI_U32      u32RHS1_MAX;
    HI_U32      u32RHS2_MAX;
...
IMX214_STATE_S g_astimx214State[ISP_MAX_PIPE_NUM] = {{0}};

/****************************************************************************
 * extern                                                                   *
 ****************************************************************************/
extern const unsigned int imx214_i2c_addr;
extern unsigned int imx214_addr_byte;
extern unsigned int imx214_data_byte;
extern void imx214_init(VI_PIPE ViPipe);
extern void imx214_exit(VI_PIPE ViPipe);
extern void imx214_standby(VI_PIPE ViPipe);
extern void imx214_restart(VI_PIPE ViPipe);
extern int  imx214_write_register(VI_PIPE ViPipe, int addr, int data);
extern int  imx214_read_register(VI_PIPE ViPipe, int addr);

/****************************************************************************
 * local variables                                                            *
 ****************************************************************************/
#define IMX214_FULL_LINES_MAX                  (0x3FFFF)
#define IMX214_FULL_LINES_MAX_2TO1_WDR         (0x966)    // considering the YOUT_SIZE and bad frame

/* ****Imx214 Register Address**** */
#define IMX214_SHS1_ADDR                       (0x3020)
#define IMX214_SHS2_ADDR                       (0x3024)
#define IMX214_GAIN_ADDR                       (0x3014)
#define IMX214_HCG_ADDR                        (0x3009)
#define IMX214_VMAX_ADDR                       (0x3018)
#define IMX214_HMAX_ADDR                       (0x301c)
#define IMX214_RHS1_ADDR                       (0x3030)
#define IMX214_RHS2_ADDR                       (0x3034)
#define IMX214_Y_OUT_SIZE_ADDR                 (0x3418)

#define IMX214_INCREASE_LINES                  (1) /* make real fps less than stand fps because NVR require */

#define IMX214_VMAX_1080P30_LINEAR             (1125+IMX214_INCREASE_LINES)
// #define IMX214_VMAX_1080P60TO30_WDR            (1156+IMX214_INCREASE_LINES)  // 12bit
#define IMX214_VMAX_1080P60TO30_WDR            (1220+IMX214_INCREASE_LINES)  // 10bit

// sensor fps mode
#define IMX214_SENSOR_1080P_30FPS_LINEAR_MODE  (1)
#define IMX214_SENSOR_1080P_30FPS_2t1_WDR_MODE (2)

#define IMX214_RES_IS_1080P(w, h)      ((w) <= 1920 && (h) <= 1080)

#define IMX214_ERR_MODE_PRINT(pstSensorImageMode,pstSnsState)\
    do{\
...
static HI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    IMX214_SENSOR_GET_CTX(ViPipe, pstSnsState);
    ...
    pstAeSnsDft->u32FullLinesMax = IMX214_FULL_LINES_MAX;
...
{
    ISP_SNS_STATE_S *pstSnsState = HI_NULL;
    HI_U32 u32VMAX = IMX214_VMAX_1080P30_LINEAR;
    ...
    switch (pstSnsState->u8ImgMode) {
        case IMX214_SENSOR_1080P_30FPS_2t1_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 15.22)) {
                u32VMAX = IMX214_VMAX_1080P60TO30_WDR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
            u32VMAX = (u32VMAX > IMX214_FULL_LINES_MAX_2TO1_WDR) ? IMX214_FULL_LINES_MAX_2TO1_WDR : u32VMAX;
            pstAeSnsDft->u32LinesPer500ms = IMX214_VMAX_1080P60TO30_WDR * 30;
            break;

        case IMX214_SENSOR_1080P_30FPS_LINEAR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 0.119)) {
                u32VMAX = IMX214_VMAX_1080P30_LINEAR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
            u32VMAX = (u32VMAX > IMX214_FULL_LINES_MAX) ? IMX214_FULL_LINES_MAX : u32VMAX;
            pstAeSnsDft->u32LinesPer500ms = IMX214_VMAX_1080P30_LINEAR * 15;
            break;
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        /*
            ...
            2n + 5
            RHS1 <= FSC - BRL*2 -21
            (2 * VMAX_IMX214_1080P30_WDR - 2 * gu32BRL - 21) - (((2 * VMAX_IMX214_1080P30_WDR - 2 * 1109 - 21) - 5) %2)
        */
        g_astimx214State[ViPipe].u32RHS1_MAX = (u32VMAX - g_astimx214State[ViPipe].u32BRL) * 2 - 21;
    } else {
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        u32FullLines = (u32FullLines > 2 * IMX214_FULL_LINES_MAX_2TO1_WDR) ? 2 * IMX214_FULL_LINES_MAX_2TO1_WDR : u32FullLines;
        pstSnsState->au32FL[0] = (u32FullLines >> 1) << 1;
        g_astimx214State[ViPipe].u32RHS1_MAX = pstSnsState->au32FL[0] - g_astimx214State[ViPipe].u32BRL * 2 - 21;
    } else {
        u32FullLines = (u32FullLines > IMX214_FULL_LINES_MAX) ? IMX214_FULL_LINES_MAX : u32FullLines;
...
{
    ISP_SNS_STATE_S *pstSnsState = HI_NULL;
    HI_U32 u32HCG = g_astimx214State[ViPipe].u8Hcg;
    HI_U32 u32Tmp;
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        */
        if (genFSWDRMode[ViPipe] == ISP_FSWDR_LONG_FRAME_MODE) {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp > (g_astimx214State[ViPipe].u32RHS1_MAX - 3)) ? (g_astimx214State[ViPipe].u32RHS1_MAX - 3) : u32IntTimeMaxTmp;
...
static HI_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    pstDef->stSensorMode.u32SensorID = IMX214_ID;
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        if (pstSnsState->u8ImgMode == IMX214_SENSOR_1080P_30FPS_LINEAR_MODE) {
            u32FullLines_5Fps = (IMX214_VMAX_1080P30_LINEAR * 30) / 5;
        } else {
            return;
        }
    }
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        imx214_write_register(ViPipe, IMX214_GAIN_ADDR, 0x00);

        imx214_write_register(ViPipe, IMX214_VMAX_ADDR, u32FullLines_5Fps & 0xFF);
        imx214_write_register(ViPipe, IMX214_VMAX_ADDR + 1, (u32FullLines_5Fps & 0xFF00) >> 8);
        imx214_write_register(ViPipe, IMX214_VMAX_ADDR + 2, (u32FullLines_5Fps & 0x30000) >> 16);

        imx214_write_register(ViPipe, IMX214_SHS1_ADDR, u32MaxIntTime_5Fps & 0xFF);
        imx214_write_register(ViPipe, IMX214_SHS1_ADDR + 1, (u32MaxIntTime_5Fps & 0xFF00) >> 8);
        imx214_write_register(ViPipe, IMX214_SHS1_ADDR + 2, (u32MaxIntTime_5Fps & 0x30000) >> 16);

    } else { /* setup for ISP 'normal mode' */
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > 0x1FFFF) ? 0x1FFFF : pstSnsState->u32FLStd;
        imx214_write_register(ViPipe, IMX214_VMAX_ADDR, pstSnsState->u32FLStd & 0xFF);
        imx214_write_register(ViPipe, IMX214_VMAX_ADDR + 1, (pstSnsState->u32FLStd & 0xFF00) >> 8);
        imx214_write_register(ViPipe, IMX214_VMAX_ADDR + 2, (pstSnsState->u32FLStd & 0x30000) >> 16);
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx214BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = imx214_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = imx214_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = imx214_data_byte;
        }

        // Linear Mode Regs
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = IMX214_SHS1_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = IMX214_SHS1_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = IMX214_SHS1_ADDR + 2;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;       // make shutter and gain effective at the same time
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = IMX214_GAIN_ADDR;  // gain
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 1;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX214_HCG_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX214_VMAX_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX214_VMAX_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX214_VMAX_ADDR + 2;
        // DOL 2t1 Mode Regs
        if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
            pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
            ...
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX214_SHS2_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX214_SHS2_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX214_SHS2_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX214_VMAX_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = IMX214_VMAX_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = IMX214_VMAX_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = IMX214_RHS1_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = IMX214_RHS1_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr = IMX214_RHS1_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr = IMX214_Y_OUT_SIZE_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr = IMX214_Y_OUT_SIZE_ADDR + 1;
        }
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (pstSensorImageMode->f32Fps <= 30) {
        if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
            if (IMX214_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                u8SensorImageMode              = IMX214_SENSOR_1080P_30FPS_LINEAR_MODE;
                pstSnsState->u32FLStd          = IMX214_VMAX_1080P30_LINEAR;
                g_astimx214State[ViPipe].u8Hcg = 0x2;
            } else {
                IMX214_ERR_MODE_PRINT(pstSensorImageMode, pstSnsState);
                return HI_FAILURE;
            }
        } else if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
            if (IMX214_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                u8SensorImageMode               = IMX214_SENSOR_1080P_30FPS_2t1_WDR_MODE;
                g_astimx214State[ViPipe].u32BRL = 1109;
                g_astimx214State[ViPipe].u8Hcg  = 0x1;
                pstSnsState->u32FLStd           = IMX214_VMAX_1080P60TO30_WDR * 2;
            } else {
                IMX214_ERR_MODE_PRINT(pstSensorImageMode, pstSnsState);
                return HI_FAILURE;
            }
...
static HI_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    pstSnsState->bSyncInit = HI_FALSE;
    pstSnsState->u8ImgMode = IMX214_SENSOR_1080P_30FPS_LINEAR_MODE;
    pstSnsState->enWDRMode = WDR_MODE_NONE;
    pstSnsState->u32FLStd = IMX214_VMAX_1080P30_LINEAR;
    pstSnsState->au32FL[0] = IMX214_VMAX_1080P60TO30_WDR;
    pstSnsState->au32FL[1] = IMX214_VMAX_1080P60TO30_WDR;
...
static HI_S32 imx214_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunImx214BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
    printf("--------------------imx214_set_bus_info%d-----------\n",unSNSBusInfo.s8I2cDev);
...
static HI_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    IMX214_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    IMX214_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static HI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ...
    IMX214_SENSOR_RESET_CTX(ViPipe);
}
...
static HI_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    stSnsAttrInfo.eSensorId = IMX214_ID;
...
static HI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(ViPipe, IMX214_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_ERR_TRACE("sensor unregister callback function failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, IMX214_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_ERR_TRACE("sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, IMX214_ID);
    if (s32Ret != HI_SUCCESS) {
...
ISP_SNS_OBJ_S stSnsImx214Obj = {
    .pfnRegisterCallback    = sensor_register_callback,
    .pfnUnRegisterCallback  = sensor_unregister_callback,
    .pfnStandby             = imx214_standby,        //ctl
    .pfnRestart             = imx214_restart,        //ctl
    .pfnMirrorFlip          = HI_NULL,
    .pfnWriteReg            = imx214_write_register, //ctl
    .pfnReadReg             = imx214_read_register,  //ctl
    .pfnSetBusInfo          = imx214_set_bus_info,
